...
foreach ($navItems as $ni) {
	...
	*/
	...
	if ($ni->hasSubmenu) {
		...
		$classes[] = 'dropdown';
		$ni->dropDownToggleClass = 'dropdown-toggle';
		$ni->dataToggle = 'data-toggle="dropdown"';
		$ni->caret = ' <b class="caret"></b>';
		$ni->url = '#';
	}
	...
	*/
	...
	$classes[] = 'nav-item-' . $ni->cObj->getCollectionHandle();
...
foreach ($navItems as $ni) {
	...
	echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="'. $ni->dropDownToggleClass .'" '. $ni->dataToggle .'>' . $ni->name . $ni->caret . '</a>';
